 ...
 * @since		1.6
 */
class TwojToolboxModelArticlesList extends JModelList
{
	...
	function getListQuery()
	{
		...
		$ordering_string = $this->getState('list.ordering', 'a.ordering');
		if( $ordering_string != 'RAND()' ) $ordering_string .=' '.$this->getState('list.direction', 'ASC');
		$query->order($ordering_string);
		$query->group('a.id');		
	 ...
	 *
	 * Overriden to inject convert the attribs field into a  object.
	 *
	...
	public function getItems()
	{
		...
		{	
			...
			$item->fulltext = preg_replace($regex, '', $item->fulltext);
