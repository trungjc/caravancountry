...
<form action="index.php?option=com_mijosef&amp;controller=bookmarks&amp;task=view" method="post" name="adminForm" id="adminForm">
	<table class="adminlist table table-striped">
		<thead>
			<tr>
				...
				<th>
					<?php echo JHTML::_('grid.sort', JText::_('COM_MIJOSEF_BOOKMARKS_NAME'), 'name', $this->lists['order_dir'], $this->lists['order']); ?>
				</th>
				<th width="60">
					<?php echo JHTML::_('grid.sort', JText::_('COM_MIJOSEF_BOOKMARKS_TYPE'), 'btype', $this->lists['order_dir'], $this->lists['order']); ?>
				</th>
				<th width="140">
					<?php echo JHTML::_('grid.sort', JText::_('COM_MIJOSEF_BOOKMARKS_PLACEHOLDER'), 'placeholder', $this->lists['order_dir'], $this->lists['order']); ?>
				</th>
				<?php if ($this->MijosefConfig->ui_bookmarks_published == 1) { ?>
				<th width="60" style="text-align: center;">
					<?php echo JHTML::_('grid.sort', JText::_('Published'), 'published', $this->lists['order_dir'], $this->lists['order']); ?>
				</th>
				<?php }	?>
				<?php if ($this->MijosefConfig->ui_bookmarks_id == 1) { ?>
		...
		for ($i=0, $n=count($this->items); $i < $n; $i++) {
			...
			switch ($row->btype) {
				case 'icon':
					$type = JText::_('COM_MIJOSEF_BOOKMARKS_TYPE_1');
					break;
				case 'iconset':
					$type = JText::_('COM_MIJOSEF_BOOKMARKS_TYPE_2');
					break;
				case 'badge':
					$type = JText::_('COM_MIJOSEF_BOOKMARKS_TYPE_3');
					break;
			}
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td>
					<?php
					$link = JRoute::_('index.php?option=com_mijosef&controller=bookmarks&task=edit&cid[]='.$row->id.'&amp;tmpl=component');?>
					<a href="<?php echo $link; ?>" style="cursor:pointer" class="modal" rel="{handler: 'iframe', size: {x: 600, y: 400}}">
						<?php echo $row->name; ?>
					</a>
				</td>
				<td align="center">
