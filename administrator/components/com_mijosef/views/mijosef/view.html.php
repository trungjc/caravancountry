...
class MijosefViewMijosef extends MijosefView {
	...
	function display($tpl = null) {
		...
		$this->toolbar->appendButton('Popup', 'help', JText::_('Help'), 'http://mijosoft.com/support/docs/mijosef/user-manual/control-panel?tmpl=component', 650, 500);
	...
	function showStatus($type) {
		...
		if (isset($status[$type])) {
			if ($type == "live_site") {
				echo '<input type="text" name="live_site" id="live_site" class="inputbox" size="35" style="width: 150px;" value="'.$status['live_site'].'" />';
				echo '&nbsp;';
				echo '<input type="button" class="btn btn-primary" style="margin-bottom: 10px;" onclick="sefStatus(\''.$type.'\', \'0\');" value="'.JText::_('Save').'" />';
			}
			elseif ($type == "php") {
				if ($status[$type]) {
			...
			elseif ($type == "version_checker") {
				if ($status[$type]) {
					echo '&nbsp;&nbsp;&nbsp;';
					echo '<input type="button" class="btn btn-danger btn-small" onclick="sefStatus(\''.$type.'\', \'0\');" value="'.JText::_('Disable').'" />';
				} else {
					echo '&nbsp;&nbsp;&nbsp;';
					echo '<input type="button" class="btn btn-success btn-small" onclick="sefStatus(\''.$type.'\', \'1\');" value="'.JText::_('Enable').'" />';
				}
			} elseif ($type == "jfrouter") {
				if ($status[$type]) {
					echo '<span style="font-weight: bold; color: red;">'.JText::_('Enabled').'</span>';
					echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
					echo '<input type="button" class="btn btn-success btn-small" onclick="sefStatus(\''.$type.'\', \'0\');" value="'.JText::_('Disable').'" />';
				} else {
					echo '<span style="font-weight: bold; color: green;">'.JText::_('Disabled').'</span>';
					echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
					echo '<input type="button" class="btn btn-danger btn-small" onclick="sefStatus(\''.$type.'\', \'1\');" value="'.JText::_('Enable').'" />';
				}
