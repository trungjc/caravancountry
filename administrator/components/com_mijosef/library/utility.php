...
class MijosefUtility {
    ...
    public function getRadioList($name, $selected, $attrs = '', $id = false, $old_style = false) {
        if (empty($attrs)) {
            $attrs = 'class="inputbox" size="2"';
        }

        if ($old_style == true) {
            $arr = array(JHtml::_('select.option', 2, JText::_('JYES')), JHtml::_('select.option', 1, JText::_('JNO')));
        }
        else {
    ...
    public function getConfigState($params, $cfg_name, $prm = "") {
		if (!is_object($params)) {
			return false;
		}
    ...
    public function getSkippedComponents() {
        return "'com_sef', 'com_sh404sef', 'com_joomfish', 'com_config', 'com_media', 'com_installer', 'com_templates', 'com_plugins', 'com_modules', 'com_cpanel', 'com_cache', 'com_messages', 'com_menus', 'com_massmail', 'com_languages', 'com_admin', 'com_login', 'com_checkin', 'com_categories', 'com_redirect', 'com_falang', 'com_joomlaupdate'";
    }
	
    public function getExtensionFromRequest() {
		static $extension;
    ...
    public function getOptionFromRealURL($url) {
		$url = str_replace('&amp;', '&', $url);
    ...
    public function getBooleanRadio($name, $attrs, $selected) {
    ...
    public function getRouterList($component) {
    ...
    public function getRemoteInfo() {
		...
		if (!is_array($information)) {
			...
			$components = self::getRemoteData('http://mijosoft.com/index.php?option=com_mijoextensions&view=xml&format=xml&catid=1');
			$extensions = self::getRemoteData('http://mijosoft.com/index.php?option=com_mijoextensions&view=xml&format=xml&catid=2');
			...
			if (strstr($components, '<?xml version="1.0" encoding="UTF-8" ?>')) {
				...
				foreach ($category->children() as $component) {
					...
					if ($option == 'com_mijosef' and ($compability == 'all' or $compability == '1.6' or $compability == '1.6_3.0' or $compability == '1.5_1.6_3.0')) {
						$information['mijosef'] = trim((string)$component->attributes()->version);
						break;
					}
				}
			}
			...
			if (strstr($extensions, '<?xml version="1.0" encoding="UTF-8" ?>')) {
				...
				foreach ($category->children() as $extension) {
					...
					if ($compability == 'all' or $compability == '1.6' or $compability == '1.6_3.0' or $compability == '1.5_1.6_3.0') {
						$ext = new stdClass();
    ...
    public function storeConfig($MijosefConfig) {
    ...
    public function _multiLayeredCheckup($section, $text, $ext_params, $area, $url_params, $component, $real_url, $own_page = false) {
		$_area = $section."_area";
    ...
    public function _getArea($param, $config) {
    ...
    public function _urlFilter($section, $sef_url, $real_url, $ext_params) {
    ...
    public function JoomfishInstalled() {
        static $installed;
    ...
    public function FalangInstalled() {
        static $installed;
	...
	// Get Menu title
    public function getMenuTitle($Itemid, $start_level = 0, $length_level = 0) {
		if (empty($Itemid)) {
			return array();
		}
	...
	// No follow links
    public function noFollow($match) {
		$uri = JFactory::getURI();
		$host = $uri->getHost();
		...
		if ($this->_getDomainFromLink($match[3]) != $host && !self::_alreadyNofollow($match[1]) && !self::_alreadyNofollow($match[4])) {
    ...
    public function _getDomainFromLink($url) {
    ...
    public function _alreadyNofollow($text) {
		return (preg_match("/rel=[\"\'].*?nofollow.*?[\"\']/i", $text )) ? true : false ;
	}
	
    public function _alreadyRelAttr($text) {
