...
// Controller Class
class MijosefControllerSefUrls extends MijosefController {
	...
	function apply() {
		...
		// Return
		parent::route(JText::_('COM_MIJOSEF_URL_SEF_SAVED'));
	}
	...
	function trash() {
		...
		// Action
		parent::updateParam($this->_table, 'MijosefSefUrls', 'params', 'trashed', 1, $this->_model);
		parent::updateParam($this->_table, 'MijosefSefUrls', 'params', 'published', 0, $this->_model);
	...
	function restore() {
		...
		// Action
		parent::updateParam($this->_table, 'MijosefSefUrls', 'params', 'trashed', 0, $this->_model);
		parent::updateParam($this->_table, 'MijosefSefUrls', 'params', 'published', 1, $this->_model);
	...
	function setTags() {
		...
		// Action
		parent::updateParam($this->_table, 'MijosefSefUrls', 'params', 'tags', $tags, $this->_model);
	...
	function setIlinks() {
		...
		// Action
		parent::updateParam($this->_table, 'MijosefSefUrls', 'params', 'ilinks', $ilinks, $this->_model);
	...
	function setBookmarks() {
		...
		// Action
		parent::updateParam($this->_table, 'MijosefSefUrls', 'params', 'bookmarks', $bookmarks, $this->_model);
	...
	function copyToMetadata() {
		...
		if (!$this->_model->copyTo('metadata')) {
			$msg = JText::_('COM_MIJOSEF_URL_SEF_RECORDS_COPIED');
		} else {
			$msg = JText::_('COM_MIJOSEF_URL_SEF_RECORDS_COPIED_NOT');
		}
	...
	function editApply() {
		...
		if (!parent::saveRecord($post, 'MijosefSefUrls', $post['id'])) {
			...
			if ($post['modal'] == '1') {
				// Return
				$this->setRedirect('index.php?option=com_mijosef&controller=sefurls&task=edit&cid[]='.$post['id'].'&tmpl=component', JTEXT::_('COM_MIJOSEF_COMMON_RECORD_SAVED'));
			} else {
				// Return
				$this->setRedirect('index.php?option=com_mijosef&controller=sefurls&task=edit&cid[]='.$post['id'], JText::_('COM_MIJOSEF_COMMON_RECORD_SAVED'));
			}
		}
	}
