...
class MijoSEF_com_contact extends MijosefExtension {
	...
	function _getItemCatId($id) {
		...
		if (!isset($cache[$id])) {
			if ($this->MijosefConfig->cache_instant == 1) {
				$rows = MijoDatabase::loadRowList("SELECT id, catid FROM #__contact_details");
				foreach ($rows as $row) {
					$cache[$row[0]] = $row[1];
				}
			} else {
				$cache[$id] = MijoDatabase::loadResult("SELECT catid FROM #__contact_details WHERE id = {$id}");
			}
		}
	...
	function _getCategory16($id) {
		...
		if (!isset($cache[$id])) {
			...
			while ($id > 1 && $id != 4 && $id != 11) {
				$row = MijoDatabase::loadObject("SELECT title, alias, parent_id, description{$joomfish} FROM #__categories WHERE id = '{$id}' AND extension = 'com_contact'");
    ...
    function getCategoryList($query) {
        ...
        $rows = MijoDatabase::loadObjectList("SELECT id, title AS name FROM #__categories WHERE {$field} = '{$value}' ORDER BY title");
