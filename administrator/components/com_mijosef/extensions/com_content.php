...
class MijoSEF_com_content extends MijosefExtension {
	...
	function _getItemCatId($id) {
		...
		if (!isset($cache[$id])) {
			if ($this->MijosefConfig->cache_instant == 1) {
				$rows = MijoDatabase::loadRowList("SELECT id, catid FROM #__content");
				foreach ($rows as $row) {
					$cache[$row[0]] = $row[1];
				}
			} else {
				$cache[$id] = MijoDatabase::loadResult("SELECT catid FROM #__content WHERE id = {$id}");
			}
		}
	...
	function _getSection($id) {
		...
		if (!isset($cache[$id])) {
			...
			$row = MijoDatabase::loadObject("SELECT title, alias, description{$joomfish} FROM #__sections WHERE id = {$id}");
    ...
    function _getCategory($id, $is_wp = false, $view = '') {
		...
		if (!isset($cache[$id])) {
			$joomfish = $this->MijosefConfig->joomfish_trans_url ? ', id' : '';
			$row = MijoDatabase::loadObject("SELECT title, alias, section, description{$joomfish} FROM #__categories WHERE id = {$id}");
	...
	function _getCategory16($id, $is_wp = false, $view = '') {
		...
		if (!isset($cache[$id])) {
			...
			while ($id > 2 && $id != 9) {
				$row = MijoDatabase::loadObject("SELECT title, alias, parent_id, description{$joomfish} FROM #__categories WHERE id = '{$id}' AND extension = 'com_content'");
    ...
    function _getArticle($id) {
		...
		if (!isset($cache[$id])) {
            ...
            if ($this->is30()) {
			    $row = MijoDatabase::loadObject("SELECT title, alias, introtext, metadesc{$joomfish} FROM #__content WHERE id = {$id}");
            }
            else {
    ...
    function _getArticleWP($id) {
		...
		if (!isset($cache[$id])) {
			$joomfish = $this->MijosefConfig->joomfish_trans_url ? ', id' : '';
			$row = MijoDatabase::loadObject("SELECT title, alias, catid, introtext, metadesc, sectionid, publish_up, created_by{$joomfish} FROM #__content WHERE id = {$id}");
