...
class MijosefModelExtensions extends MijosefModel {
	...
	function getToolbarSelections() {
        ...
        $toolbar->action = JHTML::_('select.genericlist', $act, 'ext_action', 'class="inputbox" style="width: 180px;" size="1"');
        ...
        $toolbar->selection = JHTML::_('select.genericlist', $sel, 'ext_selection', 'class="inputbox" style="width: 120px;" size="1"');
	...
	function getLists() {
		...
		// Reset filters
        $lists['reset_filters'] = '<input type="submit" class="btn btn-primary" onClick="resetFilters();" value="'. JText::_('Reset') .'" />';
        ...
        // Search prefix
        $lists['search_prefix'] = "<input type=\"text\" name=\"search_prefix\" value=\"{$this->search_prefix}\" size=\"25\" style=\"width: 100px;\" maxlength=\"255\" onchange=\"document.adminForm.submit();\" />";
		...
		$router_list[] = JHTML::_('select.option', '0', JText::_('COM_MIJOSEF_EXTENSIONS_VIEW_SELECT_DISABLE'));
   	   	$lists['router_list'] = JHTML::_('select.genericlist', $router_list, 'filter_router', 'class="inputbox" size="1" style="width: 170px;"'.$javascript,'value', 'text', $this->filter_router);
		...
		$skip_list[] = JHTML::_('select.option', '0', JText::_('No'));
   	   	$lists['skip_list'] = JHTML::_('select.genericlist', $skip_list, 'filter_skipmenu', 'class="inputbox" size="1" style="width: 100px;"'.$javascript,'value', 'text', $this->filter_skipmenu);
	...
	function _buildViewWhere() {
		...
		if ($this->filter_router != -1) {
			$src = $this->_db->escape($this->filter_router, true);
			$where[] = "params LIKE '%\"router\":{$src}%'";
		}
		
		if ($this->search_prefix) {
			$src = $this->_db->escape($this->search_prefix, true);
			$where[] = "params LIKE '%prefix={$src}%'";
		}
		
		if ($this->filter_skipmenu != -1) {
			$src = $this->_db->escape($this->filter_skipmenu, true);
