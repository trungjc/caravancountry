...
// Model Class
class MijosefModelMetadata extends MijosefModel {
	...
	function apply() {
		...
		foreach ($sef_id as $id => $val) {
			...
			// Save
			MijoDatabase::query("UPDATE #__mijosef_metadata SET title = '{$title}', description = '{$description}', keywords = '{$keywords}' WHERE id = {$id}");
		}
	}
	...
	function deleteEmptyReal($where) {
		if (!$urls = MijoDatabase::loadResultArray("SELECT url_sef FROM #__mijosef_metadata {$where}")) {
			return false;
		}
		
		foreach ($urls as $url) {
			$url_real = MijoDatabase::loadObject("SELECT url_real FROM #__mijosef_urls WHERE url_sef = '{$url}'");
			if (!is_object($url_real)) {
				MijoDatabase::query("DELETE FROM #__mijosef_metadata WHERE url_sef = '{$url}'");
			}
		}
	...
	function getToolbarSelections() {
        ...
        $act[] = JHTML::_('select.option', 'deleteemptyreal', JText::_('COM_MIJOSEF_TOOLBAR_DELETE_EMPTY_REAL'));
        ...
        $act[] = JHTML::_('select.option', 'clean', JText::_('COM_MIJOSEF_TOOLBAR_CLEAN'));
        $act[] = JHTML::_('select.option', 'update', JText::_('COM_MIJOSEF_TOOLBAR_UPDATE'));
		$act[] = JHTML::_('select.option', 'sep', '---');
        ...
        $toolbar->action = JHTML::_('select.genericlist', $act, 'meta_action', 'class="inputbox" style="width: 120px;" size="1" onchange="showInput();"');
		
		// Purge/Update
		$fields[] = JHTML::_('select.option', 'all', JText::_('COM_MIJOSEF_METADATA_ALL_FIELDS'));
		$fields[] = JHTML::_('select.option', 'title', JText::_('COM_MIJOSEF_COMMON_TITLE'));
        $fields[] = JHTML::_('select.option', 'description', JText::_('COM_MIJOSEF_COMMON_DESCRIPTION'));
        $fields[] = JHTML::_('select.option', 'keywords', JText::_('COM_MIJOSEF_COMMON_KEYWORDS'));
        //$toolbar->fields = JHTML::_('select.genericlist', $fields, 'meta_fields', 'class="inputbox" size="1"');
		$toolbar->fields = '<div id="meta_fields" style="display: none">'.JHTML::_('select.genericlist', $fields, 'tb_newfields', 'class="inputbox" style="width: 120px;" size="1"', 'value', 'text', 'all').'</div>';
        ...
        $toolbar->selection = JHTML::_('select.genericlist', $sel, 'meta_selection', 'class="inputbox" style="width: 120px;" size="1"');
	...
	function getLists() {
		...
		// Search URL
        $lists['search_url'] = "<input type=\"text\" name=\"search_url\" value=\"{$this->search_url}\" size=\"30\" style=\"width: 100px;\" maxlength=\"255\" onchange=\"document.adminForm.submit();\" />";
		...
		// Search Title
        $lists['search_title'] = "<input type=\"text\" name=\"search_title\" value=\"{$this->search_title}\" size=\"20\" style=\"width: 90px;\" maxlength=\"255\" onchange=\"document.adminForm.submit();\" />";
		
		// Title Filter
		$title_list[] = JHTML::_('select.option', '-1', JText::_('COM_MIJOSEF_COMMON_SELECT'));
		$title_list[] = JHTML::_('select.option', '1', JText::_('COM_MIJOSEF_METADATA_SELECT_EMPTY'));
		$title_list[] = JHTML::_('select.option', '2', JText::_('COM_MIJOSEF_METADATA_SELECT_FULL'));
   	   	$lists['title_list'] = JHTML::_('select.genericlist', $title_list, 'filter_title', 'class="inputbox" size="1" style="width: 80px;"'.$javascript,'value', 'text', $this->filter_title);
		
		// Search Description
        $lists['search_desc'] = "<input type=\"text\" name=\"search_desc\" value=\"{$this->search_desc}\" size=\"20\" style=\"width: 90px;\" maxlength=\"255\" onchange=\"document.adminForm.submit();\" />";
		
		// Description Filter
		$desc_list[] = JHTML::_('select.option', '-1', JText::_('COM_MIJOSEF_COMMON_SELECT'));
		$desc_list[] = JHTML::_('select.option', '1', JText::_('COM_MIJOSEF_METADATA_SELECT_EMPTY'));
		$desc_list[] = JHTML::_('select.option', '2', JText::_('COM_MIJOSEF_METADATA_SELECT_FULL'));
   	   	$lists['desc_list'] = JHTML::_('select.genericlist', $desc_list, 'filter_desc', 'class="inputbox" size="1" style="width: 80px;"'.$javascript,'value', 'text', $this->filter_desc);
		
		// Search Keywords
        if ($this->MijosefConfig->ui_metadata_keys == 1) {
       		$lists['search_key'] = "<input type=\"text\" name=\"search_key\" value=\"{$this->search_key}\" size=\"20\" style=\"width: 90px;\" maxlength=\"255\" onchange=\"document.adminForm.submit();\" />";
       		
			$key_list[] = JHTML::_('select.option', '-1', JText::_('COM_MIJOSEF_COMMON_SELECT'));
			$key_list[] = JHTML::_('select.option', '1', JText::_('COM_MIJOSEF_METADATA_SELECT_EMPTY'));
			$key_list[] = JHTML::_('select.option', '2', JText::_('COM_MIJOSEF_METADATA_SELECT_FULL'));
	   	   	$lists['key_list'] = JHTML::_('select.genericlist', $key_list, 'filter_key', 'class="inputbox" size="1" style="width: 80px;"'.$javascript,'value', 'text', $this->filter_key);
        }
		...
		if ($this->MijosefConfig->ui_metadata_id == 1) {
        	$lists['search_id'] = "<input type=\"text\" name=\"search_id\" value=\"{$this->search_id}\" size=\"3\" style=\"width: 30px;\" maxlength=\"10\" onchange=\"document.adminForm.submit();\" />";
		}
		return $lists;
	}
	...
	function getURLs() {
		...
		if (!isset($urls) && !empty($this->_data)) {
			...
			$tables = '#__mijosef_urls AS u, #__mijosef_metadata AS m';
			$where = "u.url_sef = m.url_sef AND m.id IN ({$items}) AND u.params LIKE '%\"notfound\":0%'";
			
    		$urls = MijoDatabase::loadObjectList("SELECT {$select} FROM {$tables} WHERE {$where} ORDER BY u.url_sef {$this->filter_order_Dir}");
		}
	...
	function _buildViewWhere($prefix = "") {
		...
		if ($this->filter_component != '-1') {
			$src = $this->_db->escape($this->filter_component);
