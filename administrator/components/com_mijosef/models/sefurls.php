...
// Model Class
class MijosefModelSefUrls extends MijosefModel {
	...
	function apply() {
		...
		foreach ($url_id as $id => $val) {
			MijoDatabase::query("UPDATE #__mijosef_urls SET url_sef = '{$url_sef[$id]}', url_real = '{$url_real[$id]}', used = '{$url_used[$id]}' WHERE id = {$id}");
			
			$row = Mijosef::getTable('MijosefSefUrls');
			if ($row->load($id)) {
    ...
    function generateURLs() {
		$urls = MijoDatabase::loadObjectList("SELECT DISTINCT url_sef FROM #__mijosef_urls WHERE used != 1 AND params LIKE '%\"visited\":1%' AND params LIKE '%\"notfound\":0%' AND params LIKE '%\"trashed\":0%' LIMIT 500");
        ...
        // Visit the homepage
        Mijosef::get('utility')->getRemoteData(JURI::root());
		
		$visited = array();
		for ($i = 0; $i < 4; $i ++) {
			$urls = MijoDatabase::loadObjectList("SELECT DISTINCT url_sef FROM #__mijosef_urls WHERE used != 1 AND params LIKE '%\"visited\":0%' AND params LIKE '%\"notfound\":0%' AND params LIKE '%\"trashed\":0%' LIMIT 500");
			...
			foreach ($urls as $sef_url) {
				...
				Mijosef::get('utility')->getRemoteData($url);
	...
	function getToolbarSelections() {
		...
		if ($this->type == 'trashed') {
	        ...
	        $act[] = JHTML::_('select.option', 'restore', JText::_('COM_MIJOSEF_TOOLBAR_RESTORE'));
		} else {
	        $act[] = JHTML::_('select.option', 'delete', JText::_('Delete'));
			$act[] = JHTML::_('select.option', 'sep', '---');
	        $act[] = JHTML::_('select.option', 'trash', JText::_('COM_MIJOSEF_TOOLBAR_TRASH'));
			$act[] = JHTML::_('select.option', 'sep', '---');
			...
			if ($this->MijosefConfig->ui_sef_used == 1) {
		        $act[] = JHTML::_('select.option', 'used', JText::_('COM_MIJOSEF_TOOLBAR_USE'));
		        $act[] = JHTML::_('select.option', 'resetused', JText::_('COM_MIJOSEF_TOOLBAR_USE_RESET'));
				$act[] = JHTML::_('select.option', 'sep', '---');
			}
			if ($this->MijosefConfig->ui_sef_locked == 1) {
		        $act[] = JHTML::_('select.option', 'lock', JText::_('COM_MIJOSEF_TOOLBAR_LOCK'));
		        $act[] = JHTML::_('select.option', 'unlock', JText::_('COM_MIJOSEF_TOOLBAR_LOCK_UN'));
				$act[] = JHTML::_('select.option', 'sep', '---');
			}
			if ($this->MijosefConfig->ui_sef_blocked == 1) {
		        $act[] = JHTML::_('select.option', 'block', JText::_('COM_MIJOSEF_TOOLBAR_BLOCK'));
		        $act[] = JHTML::_('select.option', 'unblock', JText::_('COM_MIJOSEF_TOOLBAR_BLOCK_UN'));
				$act[] = JHTML::_('select.option', 'sep', '---');
			}
	        ...
	        $act[] = JHTML::_('select.option', 'settags', JText::_('COM_MIJOSEF_TOOLBAR_SET_TAGS'));
	        $act[] = JHTML::_('select.option', 'setilinks', JText::_('COM_MIJOSEF_TOOLBAR_SET_ILINKS'));
	        $act[] = JHTML::_('select.option', 'setbookmarks', JText::_('COM_MIJOSEF_TOOLBAR_SET_BOOKMARKS'));
	        $act[] = JHTML::_('select.option', 'setunpublishtag', JText::_('COM_MIJOSEF_TOOLBAR_SET_TAG_UNPUBLISH'));
			$act[] = JHTML::_('select.option', 'sep', '---');
	        $act[] = JHTML::_('select.option', 'copytometadata', JText::_('COM_MIJOSEF_TOOLBAR_COPY_TO_METADATA'));
	        $act[] = JHTML::_('select.option', 'copytositemap', JText::_('COM_MIJOSEF_TOOLBAR_COPY_TO_SITEMAP'));
			$act[] = JHTML::_('select.option', 'sep', '---');
	        $act[] = JHTML::_('select.option', 'backup', JText::_('COM_MIJOSEF_TOOLBAR_BACKUP'));
		}
        $toolbar->action = JHTML::_('select.genericlist', $act, 'sefurls_action', 'class="inputbox" style="width: 170px;" size="1" onchange="showInput();"');
        ...
        $toolbar->selection = JHTML::_('select.genericlist', $sel, 'sefurls_selection', 'class="inputbox" style="width: 120px;" size="1"');
	...
	function delete($where) {
		...
		if ($this->MijosefConfig->delete_other_sef == '1') {
			$sef_urls = MijoDatabase::loadColumn("SELECT url_sef FROM #__mijosef_{$this->_table}{$where}");
		}
		
		if (!MijoDatabase::query("DELETE FROM #__mijosef_{$this->_table}{$where}")) {
			return false;
		}
		
		if (!empty($sef_urls)) {
			foreach ($sef_urls as $sef_url) {
				MijoDatabase::query("DELETE FROM #__mijosef_metadata WHERE url_sef = '{$sef_url}'");
				MijoDatabase::query("DELETE FROM #__mijosef_sitemap WHERE url_sef = '{$sef_url}'");
			}
		}
	...
	function copyTo($table) {
		...
		foreach ($urls as $url) {
			MijoDatabase::query("INSERT IGNORE INTO #__mijosef_{$table} (url_sef) VALUES ('{$url}')");
		}
	...
	function _getURLs() {
		...
		if (!$urls = MijoDatabase::loadColumn("SELECT url_sef FROM #__mijosef_urls {$where}")) {
			return false;
		}
	...
	function getLists() {
		...
		// Search SEF URL
        $lists['search_sef'] = "<input type=\"text\" name=\"search_sef\" value=\"{$this->search_sef}\" size=\"50\" maxlength=\"255\" style=\"width: 30%; margin-bottom: 15px !important;\" onchange=\"document.adminForm.submit();\" />";
		...
		// Languages list
        if ($this->MijosefConfig->ui_sef_language == 1) {
			$lang_list = array();
			$lang_list[] = JHTML::_('select.option', '-1', JText::_('COM_MIJOSEF_COMMON_LANGUAGE_FILTER'));
			$lang_list = array_merge($lang_list, Mijosef::get('utility')->getLanguages());

	        $lists['lang_list'] = JHTML::_('select.genericlist', $lang_list, 'filter_lang', 'class="inputbox" size="1" style="width: 150px;"'.$javascript, 'value', 'text', $this->filter_lang);
        }
        
		// Search Real URL
        $lists['search_real'] = "<input type=\"text\" name=\"search_real\" value=\"{$this->search_real}\" size=\"20\" maxlength=\"255\" style=\"width: 30%; margin-bottom: 15px !important;\" onchange=\"document.adminForm.submit();\" />";
        ...
        if ($this->MijosefConfig->ui_sef_published == 1) {
			...
			$used_list[] = JHTML::_('select.option', '1', JText::_('No'));
	   	   	$lists['used_list'] = JHTML::_('select.genericlist', $used_list, 'filter_used', 'class="inputbox" size="1" style="width: 80px;"'.$javascript,'value', 'text', $this->filter_used);
   	   	}
   	   	
		// Locked Filter
   	   	if ($this->MijosefConfig->ui_sef_locked == 1) {
			$locked_list[] = JHTML::_('select.option', '-1', JText::_('COM_MIJOSEF_COMMON_SELECT'));
			$locked_list[] = JHTML::_('select.option', '1', JText::_('Yes'));
			$locked_list[] = JHTML::_('select.option', '0', JText::_('No'));
	   	   	$lists['locked_list'] = JHTML::_('select.genericlist', $locked_list, 'filter_locked', 'class="inputbox" size="1" style="width: 80px;"'.$javascript,'value', 'text', $this->filter_locked);
   	   	}
   	   	
		// Blocked Filter
   	   	if ($this->MijosefConfig->ui_sef_blocked == 1) {
	   	   	$blocked_list[] = JHTML::_('select.option', '-1', JText::_('COM_MIJOSEF_COMMON_SELECT'));
			$blocked_list[] = JHTML::_('select.option', '1', JText::_('Yes'));
			$blocked_list[] = JHTML::_('select.option', '0', JText::_('No'));
	   	   	$lists['blocked_list'] = JHTML::_('select.genericlist', $blocked_list, 'filter_blocked', 'class="inputbox" size="1" style="width: 80px;"'.$javascript,'value', 'text', $this->filter_blocked);
   	   	}
		...
		if ($this->MijosefConfig->ui_sef_date == 1) {
			...
			$date_list[] = JHTML::_('select.option', 'c', JText::_('COM_MIJOSEF_URL_SEF_SELECT_CREATED'));
			$date_list[] = JHTML::_('select.option', 'm', JText::_('COM_MIJOSEF_URL_SEF_SELECT_MODIFIED'));
	   	   	$lists['date_list'] = JHTML::_('select.genericlist', $date_list, 'filter_date', 'class="inputbox" size="1" style="width: 100px;"'.$javascript, 'value', 'text', $this->filter_date);
		}
		
		// Search hits
		if ($this->MijosefConfig->ui_sef_hits == 1) {
        	$lists['search_hits'] = "<input type=\"text\" name=\"search_hits\" value=\"{$this->search_hits}\" size=\"3\" maxlength=\"10\" style=\"width: 30px;\" onchange=\"document.adminForm.submit();\" />";
		}
		
		// Search id
		if ($this->MijosefConfig->ui_sef_id == 1) {
        	$lists['search_id'] = "<input type=\"text\" name=\"search_id\" value=\"{$this->search_id}\" size=\"3\" maxlength=\"10\" style=\"width: 30px;\" onchange=\"document.adminForm.submit();\" />";
		}
	...
	function used($id) {
		...
		MijoDatabase::query("UPDATE #__mijosef_urls SET used = 1 WHERE url_sef = '{$row->url_sef}' AND id != {$row->id}");
	}
	...
	function _buildViewWhere() {	
		...
		// Type Filter
		$type = $this->_db->escape($this->type, true);
		if (isset($type)) {
			if ($type != 'trashed') {
			...
			switch ($type) {
				...
				case 'notfound':
					...
					break;
				case 'locked':
					$where[] = "params LIKE '%\"locked\":1%'"; // Locked URLs
					break;
				case 'blocked':
		...
		if ($this->filter_lang != '-1') {
			$src = $this->_db->escape($this->filter_lang, true);
			$where[]= "(LOWER(url_real) LIKE '%lang={$src}&%' OR url_real LIKE '%lang={$src}')";
		}
		
		// Published Filter
		if ($this->filter_published != '-1') {
			$src = $this->_db->escape($this->filter_published, true);
		...
		if ($this->filter_locked != '-1') {
			$src = $this->_db->escape($this->filter_locked, true);
			$where[] = "params LIKE '%\"locked\":{$src}%'";
		}
		
		// Blocked Filter
		if ($this->filter_blocked != '-1') {
			$src = $this->_db->escape($this->filter_blocked, true);
		...
		$fromdate = $this->_db->escape($this->fromdate, true);
		if ($fromdate) {
			$dt = explode('.', $fromdate);
		...
		$todate = $this->_db->escape($this->todate, true);
		if ($todate) {
			$dt = explode('.', $todate);
		...
		$filter_date = $this->_db->escape($this->filter_date, true);
		if ($filter_date == 'c') {
			// From only
	...
	function getEditDuplicates() {
		...
		// Get URLs
		$rows = MijoDatabase::loadObjectList("SELECT * FROM #__mijosef_urls WHERE url_sef IN (SELECT url_sef FROM #__mijosef_urls WHERE id = '{$id}') ORDER BY url_real");
	...
	function _modifyPostData(&$post) {
		...
		// Params
        $prms = array();
        $prms['custom'] = (int)$post['url_custom'];
        $prms['published'] = (int)$post['url_published'];
        $prms['locked'] = (int)$post['url_locked'];
        $prms['blocked'] = (int)$post['url_blocked'];
        $prms['trashed'] = (int)$post['url_trashed'];
        $prms['notfound'] = (int)$post['url_notfound'];
        $prms['tags'] = (int)$post['url_tags'];
        $prms['ilinks'] = (int)$post['url_ilinks'];
        $prms['bookmarks'] = (int)$post['url_bookmarks'];
        $prms['visited'] = 0;
        $prms['notes'] = $post['url_notes'];
	...
	function _saveAliases($aliases) {
		...
		if (!empty($url_alias)) {
			...
			MijoDatabase::query("DELETE FROM #__mijosef_urls_moved WHERE url_new = '{$url_sef}'");
			...
			foreach ($urls as $url_old) {
				...
				if (!empty($url_old)) {
					MijoDatabase::query("INSERT IGNORE INTO #__mijosef_urls_moved (url_old, url_new) VALUES ('{$url_old}', '{$url_sef}')");
				}
			}
		}
	}
	...
	function _saveMetadata($post) {
		if ($post['id'] == "") {
			...
			MijoDatabase::query("INSERT IGNORE INTO #__mijosef_metadata (url_sef, published, title, description, keywords, lang, robots, googlebot, canonical) VALUES {$values}");
		} else {
			...
			MijoDatabase::query("UPDATE #__mijosef_metadata SET {$fields} WHERE id = {$post['id']}");
		}
	}
