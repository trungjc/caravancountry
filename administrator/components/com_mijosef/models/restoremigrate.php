...
// Model Class
class MijosefModelRestoreMigrate extends MijosefModel {
	...
	function _backupGetVars() {
		...
		foreach ($items as $item) {
			if (JRequest::getVar('backup_'.$item, 0, 'post')) {
				...
				$class = new MijosefBackupRestore(array('_table' => $_table, '_where' => ''));
	...
	function _restoreGetPregLine($line) {
		...
		$items = array('sefurls', 'movedurls', 'metadata', 'sitemap', 'tags', 'ilinks', 'bookmarks', 'acesefurls', 'acemovedurls', 'acemetadata', 'acesitemap', 'acetags', 'aceilinks', 'acebookmarks', 'joomsef', 'shUrl', 'shMetadata', 'sh2', 'shAliases');
		
		foreach ($items as $item) {
			if (JRequest::getVar('restore_'.$item, 0, 'post')) {
				$class = new MijosefBackupRestore();
