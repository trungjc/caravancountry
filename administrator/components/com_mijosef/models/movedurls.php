...
class MijosefModelMovedUrls extends MijosefModel {
	...
	function getToolbarSelections() {
        ...
        $toolbar->action = JHTML::_('select.genericlist', $act, 'movedurls_action', 'class="inputbox" style="width: 120px;" size="1"');
        ...
        $toolbar->selection = JHTML::_('select.genericlist', $sel, 'movedurls_selection', 'class="inputbox" style="width: 120px;" size="1"');
	...
	function getLists() {		
        ...
        if ($this->MijosefConfig->ui_moved_hits == 1) {
			...
			$hit_val[] = JHTML::_('select.option', '2', '<');
	   	   	$lists['hit_val'] = JHTML::_('select.genericlist', $hit_val, 'filter_hit_val', 'class="inputbox" size="1" style="width: 50px;"'.$javascript, 'value', 'text', $this->filter_hit_val);

			// Search hit
        	$lists['search_hit'] = "<input type=\"text\" name=\"search_hit\" value=\"{$this->search_hit}\" size=\"3\" style=\"width: 30px;\" maxlength=\"10\" onchange=\"document.adminForm.submit();\" />";
        }
        
		// Date
        if ($this->MijosefConfig->ui_moved_clicked == 1) {
			$lists['filter_date'] = JHTML::_('calendar', $this->filter_date, 'filter_date', 'filter_date', '%Y-%m-%d', array('class'=>'inputbox', 'size'=>'10', 'style'=>'width: 80px;', 'onchange'=>'document.adminForm.submit();',  'maxlength'=>'20'));
        }
